...
bool HardwareInterface::init(ros::NodeHandle& root_nh, ros::NodeHandle& robot_hw_nh)
{
  ...
  // 
  custom_state_pub_.reset(new realtime_tools::RealtimePublisher<arm_msgs::ManipulatorState>(robot_hw_nh, "arm_state", 1));
  custom_state_pub_->msg_.q_target.resize(6);
  custom_state_pub_->msg_.qd_target.resize(6);
  custom_state_pub_->msg_.i_target.resize(6);
  custom_state_pub_->msg_.q_actual.resize(6);
  custom_state_pub_->msg_.qd_actual.resize(6);
  custom_state_pub_->msg_.i_actual.resize(6);
  custom_state_pub_->msg_.tcp_force.resize(6);
  custom_state_pub_->msg_.tool_vector_target.resize(6);
  custom_state_pub_->msg_.tool_vector_actual.resize(6);
  custom_state_pub_->msg_.tcp_speed_target.resize(6);
  custom_state_pub_->msg_.tcp_speed_actual.resize(6);
  custom_state_pub_->msg_.motor_temperatures.resize(6);
  custom_state_pub_->msg_.m_target.resize(6);
  custom_state_pub_->msg_.joint_modes.resize(6);
  custom_state_pub_->msg_.tool_acc_values.resize(3);
  custom_state_pub_->msg_.qdd_target.resize(6);
...
void HardwareInterface::read(const ros::Time& time, const ros::Duration& period)
{
  ...
  if (data_pkg)
  {
    ...
    readData(data_pkg, "target_moment", target_moment_);
    readData(data_pkg, "joint_mode", joint_modes_);
    readData(data_pkg, "timestamp", timestamp_);
    readData(data_pkg, "actual_tool_accelerometer", actual_tool_accelerometer_);
    readData(data_pkg, "target_qdd", qdd_target_);
...
void HardwareInterface::publishCustomData(const ros::Time& timestamp) {
  if (custom_state_pub_->trylock())
    {
      custom_state_pub_->msg_.header.stamp = timestamp;
      for (size_t i = 0; i < target_joint_positions_.size(); ++i)
      {
        custom_state_pub_->msg_.q_target[i] = target_joint_positions_[i];
      }
      for (size_t i = 0; i < target_joint_velocities_.size(); ++i)
      {
        custom_state_pub_->msg_.qd_target[i] = target_joint_velocities_[i];
      }
      for (size_t i = 0; i < target_joint_efforts_.size(); ++i)
      {
        custom_state_pub_->msg_.i_target[i] = target_joint_efforts_[i];
      }
      for (size_t i = 0; i < target_moment_.size(); ++i)
      {
        custom_state_pub_->msg_.m_target[i] = target_moment_[i];
      }
      // tau_target
      for (size_t i = 0; i < joint_positions_.size(); ++i)
      {
        custom_state_pub_->msg_.q_actual[i] = joint_positions_[i];
      }
      for (size_t i = 0; i < joint_velocities_.size(); ++i)
      {
        custom_state_pub_->msg_.qd_actual[i] = joint_velocities_[i];
      }
      for (size_t i = 0; i < joint_efforts_.size(); ++i)
      {
        custom_state_pub_->msg_.i_actual[i] = joint_efforts_[i];
      }
      // tau_actual
      for (size_t i = 0; i < fts_measurements_.size(); ++i)
      {
        custom_state_pub_->msg_.tcp_force[i] = fts_measurements_[i];
      }
      for (size_t i = 0; i < target_tcp_pose_.size(); ++i)
      {
        custom_state_pub_->msg_.tool_vector_target[i] = target_tcp_pose_[i];
      }
      for (size_t i = 0; i < tcp_pose_.size(); ++i)
      {
        custom_state_pub_->msg_.tool_vector_actual[i] = tcp_pose_[i];
      }
      for (size_t i = 0; i < target_tcp_speed_.size(); ++i)
      {
        custom_state_pub_->msg_.tcp_speed_target[i] = target_tcp_speed_[i];
      }
      for (size_t i = 0; i < tcp_speed_.size(); ++i)
      {
        custom_state_pub_->msg_.tcp_speed_actual[i] = tcp_speed_[i];
      }
      for (size_t i = 0; i < joint_temperatures_.size(); ++i)
      {
        custom_state_pub_->msg_.motor_temperatures[i] = joint_temperatures_[i];
      }
      for (size_t i = 0; i < joint_modes_.size(); ++i)
      {
        custom_state_pub_->msg_.joint_modes[i] = joint_modes_[i];
      }
      custom_state_pub_->msg_.controller_timer = timestamp_;
      for (size_t i = 0; i < qdd_target_.size(); ++i)
      {
        custom_state_pub_->msg_.qdd_target[i] = qdd_target_[i];
      }
      for (size_t i = 0; i < actual_tool_accelerometer_.size(); ++i)
      {
        custom_state_pub_->msg_.tool_acc_values[i] = actual_tool_accelerometer_[i];
      }
      custom_state_pub_->msg_.robot_mode = robot_mode_;
      // m_actual
      custom_state_pub_->unlockAndPublish();
    // readData(data_pkg, "target_speed_fraction", target_speed_fraction_);
    // readData(data_pkg, "speed_scaling", speed_scaling_);
    // readData(data_pkg, "runtime_state", runtime_state_);
    // readData(data_pkg, "standard_analog_input0", standard_analog_input_[0]);
